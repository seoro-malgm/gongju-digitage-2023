  ...
  <div>
    <b-form @submit.prevent="id ? updateItem() : submitItem()">
      ...
      <div class="mt-4">
        <admin-editor
          title="내용"
          desc="FAQ 제목 클릭시 표시할 상세 내용을 작성합니다"
...
export default {
  ...
  methods: {
    ...
    async init() {
      if (!this.id) return;
      try {
        const data = await this.$firebase().getBoardItem("faq", [
          "id",
          this.id,
        ]);
        if (data) {
    ...
    async submitItem() {
      ...
      try {
        ...
        const data = await this.$firebase().addBoardItem("faq", {
          ...this.form,
          id,
          createdAt: new Date().toLocaleString(),
          updateDate: new Date().toLocaleString(),
