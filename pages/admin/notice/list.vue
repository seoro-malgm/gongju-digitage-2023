<template>
  <div>
    <header class="bg-gray-100 p-3 rounded-lg mb-4">
      <b-btn variant="primary text-white" to="/admin/notice/write">
        + 새 공지사항</b-btn
      >
    </header>
    <section>
...
export default {
  ...
  methods: {
    async getItems() {
      ...
      this.items = await getAllBoardItems("notice", null, null, [
        "createdAt",
        "desc",
      ]);
    },
    async removeItem(item, index) {
      const bool = await window.confirm(
        `공지사항을 <strong>"${item.title}"</strong>를 삭제하시겠습니까?`
      );
      if (bool) {
        try {
          const data = await this.$firebase().removeBoardItem("notice", [
            "id",
            item.id,
          ]);
          if (data) {
